...
async function nugetInstall(nugetSource, packageName, version, targetDir) {
    ...
    if (selectedFeed.authenticated) {
        if (!feedPAT) {
            throw new Error(
                `nuget feed ${nugetSource} requires authN but neither '--feedToken' argument nor env var 'AZ_DevOps_Read_PAT' was defined!`
            );
        }
    ...
    if (!res.ok) {
        ...
        throw new Error(
            `Cannot download ${res.url}, status: ${res.statusText} (${
                res.status
            }), body: ${body ? body.toString("ascii") : "<empty>"}`
        );
    }
    ...
    return new Promise((resolve, reject) => {
        res.body
            .pipe(fs.createWriteStream(localNupkg))
            .on("close", () => {
                resolve();
            })
            .on("error", (err) => {
        ...
        .pipe(
            eslint.results((results) => {
                if (results.warningCount > 0) {
                    throw new Error(
                        `Found ${results.warningCount} eslint errors.`
                    );
                }
            })
        )
...
function testWeb() {
    return gulp.src(["src/web/test/unit/**/*.ts"], { read: false }).pipe(
        mocha({
...
async function setGitAuthN() {
    ...
    if (!repoToken) {
        throw new Error(
            `Must specify parameter --repoToken with read and push rights to ${repoUrl}!`
        );
    }
    const bearer = `AUTHORIZATION: basic ${Buffer.from(
        `PAT:${repoToken}`
    ).toString("base64")}`;
    await git([
        "config",
...
async function snapshot() {
    ...
    const repoUrl = (
        await git(["remote", "get-url", "--all", "origin"])
    ).stdout.trim();
    ...
    try {
        ...
        if (headBranch == targetBranch) {
            throw new Error(
                `Cannot snapshot into default HEAD branch: ${headBranch}`
            );
        }
        const sourceSpec = sourceSpecParam || `origin/${headBranch}`;
        log.info(
            `  > snap shotting '${sourceSpec}' into branch: ${targetBranch}...`
        );
        await git(["checkout", headBranch]);
        const snapshotTag = `snapshot-${targetBranch
            .replace("/", "_")
            .replace(" ", "_")}-${moment.utc().format("YYMMDD[Z]HHmmss")}`;
        // await git(['tag', snapshotTag, sourceSpec]);
        await git(["checkout", "--force", "-B", targetBranch]);
        const resetMsg = (
            await git(["reset", "--hard", `"${sourceSpec}"`])
        ).stdout.trim();
        log.info(`  > snapshot (${snapshotTag}): ${resetMsg}`);
        log.info(`  > pushing snapshot branch '${targetBranch} to origin...`);
        const pushMsg = (
            await git(["push", "--force", "--tags", "origin", targetBranch])
        ).stderr.trim();
        log.info(`  > ${pushMsg}`);
    } finally {
...
const recompile = gulp.series(
    ...
    async () =>
        nugetInstall(
            feedName,
            "Microsoft.PowerApps.CLI.Core.linux-x64",
            cliVersion,
            path.resolve(distdir, "pac")
        ),
    translationsExport,
...
async function translationsExport() {
    return gulp
        ...
        .pipe(
            nls.createXlfFiles("translations-export", translationExtensionName)
        )
            ...
            gulp
                .src(
                    path.join(
                        "loc",
                        "translations-import",
                        `vscode-powerplatform.${language.id}.xlf`
                    )
                )
